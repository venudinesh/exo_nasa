...
const generateWeatherEvents = (planet: Planet): WeatherEvent[] => {
  ...
  if (planet.pl_orbper && planet.pl_orbper < 10) {
    events.push({
      ...
      description: 'Extreme temperature differences create massive storm systems at day/night boundary',
      probability: 95,
      severity: 'extreme',
...
const generateAtmosphericConditions = (planet: Planet, timeOfDay: number): AtmosphericCondition => {
  ...
  const humidity = Math.max(0, Math.min(100, 
    temperature > 273 && temperature < 373 ? 60 : 5
  ));
...
const WeatherSimulator: React.FC<WeatherSimulatorProps> = ({ 
  ...
  const updateWeatherPattern = useCallback(() => {
    ...
    if (events.length > 0) {
      ...
      if (shouldOccur) {
        const weatherTypes: WeatherPattern['type'][] = ['storm', 'extreme', 'aurora', 'rain', 'cloudy'];
